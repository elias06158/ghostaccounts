...
export function Header({ user, locale }: HeaderProps) {
  ...
  const navLinks = user
    ? [
        ...
        { href: `/${locale}/dashboard/accounts`, label: t("nav.accounts") },
        { href: `/${locale}/dashboard/scan`, label: t("nav.scan") },
        { href: `/${locale}/dashboard/settings`, label: t("nav.settings") },
      ]
    : [
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 h-16">
        {/* Logo */}
        <Link href={`/${locale}`} className="group flex items-center gap-2.5 font-bold text-foreground">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-xl px-4 py-4 flex flex-col gap-1.5">
          ...
          {user ? (
            <button
              onClick={handleLogout}
              className="mt-1 flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
            >
              <LogOut className="w-3.5 h-3.5" />
              {t("nav.logout")}
            </button>
